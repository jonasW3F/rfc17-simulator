    ...
    <div className="rounded-xl border border-line bg-surface p-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <label className="flex flex-col gap-1 col-span-3 md:col-span-1">
          <span className="text-xs text-fg-2">Bidder ID</span>
          <input
            type="text"
            value={id}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">WTP (DOT)</span>
          <NumInput
            value={wtp}
            min={1}
            step={1}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">Cores</span>
          <NumInput
            value={quantity}
            min={1}
            step={1}
      ...
      <button
        onClick={submit}
        className="mt-3 w-full rounded-md bg-ink px-3 py-1.5 text-sm font-semibold text-white hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600"
      >
        Add bidder
      </button>
    </div>
  ...
  return (
    <div className="rounded-xl border border-line bg-surface p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">Count</span>
          <NumInput
            value={count}
            min={1}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">ID prefix</span>
          <input
            type="text"
            value={prefix}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">WTP min</span>
          <NumInput
            value={wtpMin}
            onChange={setWtpMin}
            className="rounded-md border border-line px-2 py-1 text-sm font-mono"
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">WTP max</span>
          <NumInput
            value={wtpMax}
            onChange={setWtpMax}
            className="rounded-md border border-line px-2 py-1 text-sm font-mono"
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">Cores each</span>
          <NumInput
            value={qty}
            min={1}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs text-fg-2">Distribution</span>
          <select
            value={distribution}
...
function StagedList(props: {
  ...
  if (props.bidders.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-line bg-surface-2 p-6 text-center text-sm text-fg-2">
  ...
  return (
    <div className="rounded-xl border border-line bg-surface">
      <div className="border-b border-line px-4 py-2 text-sm font-semibold uppercase tracking-wide text-fg-2">
      ...
      <div className="max-h-96 overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {props.bidders.map(b => (
              <tr key={b.uiKey} className="border-t border-line">
                <td className="px-4 py-1.5 font-mono">{b.id}</td>
                <td className="px-4 py-1.5">
                  {props.tenants[b.id] ? (
                    <span className="rounded-full bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 text-xs font-medium text-amber-900 dark:text-amber-200">
                ...
                <td className="px-4 py-1.5">
                  <NumInput
                    ...
                    className="w-24 rounded-md border border-line px-2 py-0.5 font-mono"
                  />
                </td>
                <td className="px-4 py-1.5">
                  <NumInput
                    ...
                    className="w-20 rounded-md border border-line px-2 py-0.5 font-mono"
                  />
                </td>
