...
- **Genuine-demand gate on expansion (simulator extension beyond the amendment).** The amendment expands supply on any 100%-consumption round; this simulator additionally requires `reserve_price > P*` (the validator floor). This is a deliberate design decision by the user — it is *not* in `resources/rfc17-amendment.md`, so do **not** "correct" it back to the spec. It defends against validators buying cores to inflate their own active set. Validators are modelled as income-earners (both validator economic params are protocol-paid income, never expenses), so being active is pure profit; the gate is what stops them manufacturing demand below `P*`. The regression scenarios in `resources/scenarios/` set `VALIDATOR_PROFIT_MARGIN: 0` to disable the gate and test the base supply rule in isolation.
...
4. **Supply update (amendment, asymmetric, with genuine-demand gate).** After step 3: expansion fires only when consumption is at 100% **and** `reserve_price > P*` (the validator floor — see *Default parameters*). When both hold, set `next_num_cores = ceil(cores_sold / POST_EXPANSION_CONSUMPTION)` (default 0.9) so the next round lands at ~90% consumption — above the price target, preserving the price signal. Otherwise (including saturated rounds where `reserve_price ≤ P*`), compute the rolling average of `cores_sold` over the last `SCALE_DOWN_WINDOW` rounds (default 3, inclusive of the current round) and set `memory_target = ceil(avg_sold / TARGET_CONSUMPTION_RATE)`; the next supply is `min(num_cores, memory_target)` — the contraction branch can never expand (after a saturated round it holds). Final result is clamped to `[max(renewals, MIN_CORES), MAX_CORES]`. The gate keys off the **reserve** (not the clearing) price: a bidder can only win when `reserve ≤ WTP`, so once `reserve_price > P*` a validator cluster (which never values a core above `P*`) is locked out of the auction entirely. This guarantees the saturation is genuine **and** that the reserve stays above `P*` right after an expansion — so validators cannot scoop the freed slack cheaply the next round (the failure mode a clearing-based gate would leave open). Cost: the first expansion is slower, since the sticky reserve must climb to `P*` first.
...
| `P_MIN` | 1 DOT | RFC-17 |
| `MIN_INCREMENT` | 300 DOT | RFC-17 proposes 100; raised to 300 so a saturated reserve climbs past the validator floor P* in ~2 rounds instead of ~5, shortening the genuine-demand detection lag |
...
| `val_per_core` | 5 | amendment — load-bearing in `active_validators` and the validator floor `P*` |
...
| `VALIDATOR_PROFIT_MARGIN` | 0.2 | simulator — free-capital fraction of a validator's payout; sets the validator floor `P*` and the genuine-demand gate |

All are governance-adjustable; the simulator should treat them as configuration, not constants.

**Validator floor `P*` and the genuine-demand gate.** `P* = val_per_core × VALIDATOR_PROFIT_MARGIN × payout_per_validator`, where `payout_per_validator = STAKE_INCENTIVES_DOT_PER_VALIDATOR + REWARD_FOR_OPERATIONAL_COSTS_USD_PER_VALIDATOR / DOT_USD_RATE`. It is the most a homogeneous validator cluster will pay per core to activate itself, and it is **static in `num_cores`** (no reward dilution is modelled). Supply expansion is gated on `reserve_price > P*` — see step 4. (Reserve, not clearing: once the reserve exceeds `P*`, validators are priced out of the auction entirely, which also stops them from scooping freed slack right after an expansion.) Setting `VALIDATOR_PROFIT_MARGIN = 0` makes `P* = 0`, disabling the gate (used by the regression scenarios, which predate it).
